...
class PsychoacousticModel():
    ...
    def amplitude_to_dB(self,mdct_amplitude):
        ...
        Args:
            mdct_amplitude (tensor): normalized amplitudes to convert to dB scale

        Returns:
            tensor: amplitudes in dB scale in[_dB_MIN,_dB_MaX]  dtype = compute dtype
        """
        ampl_dB = (10*torch.log10(torch.max(self._INTENSITY_EPS, mdct_amplitude ** 2)) + self._dB_MAX).type(self.compute_dtype)
        return ampl_dB
     
    def amplitude_to_dB_norm(self,mdct_amplitude):
        """Utility fonction to convert normalized amplitude in [-1,1] to the normalized dB scale in [0,1]

        Args:
            mdct_amplitude (tensor): amplitude normalized in [-1,1]
            
        Returns :
            tensor : amplitudes in normalized dB scale in [0,1]   dtype = compute dtype
    ...
    def tonality(self, mdct_amplitudes):
        ...
        Args:
            mdct_amplitudes (tensor): mdct amplitudes (spectrum) for each filter. Shape : [batches_n, blocks_n, filter_bands_n, channels_n]
        ...
        geometric_mean = torch.exp(torch.mean(torch.log(torch.max(self._INTENSITY_EPS, mdct_intensity)), axis=2, keepdim=True))
        arithmetic_mean = torch.mean(mdct_intensity, axis = 2, keepdim = True)
        sfm = 10. * torch.log(torch.divide(geometric_mean,arithmetic_mean + self._INTENSITY_EPS)) / torch.log(torch.tensor(10))

        sfm = torch.minimum(sfm / -60., torch.tensor(1.0))

        return sfm
    
    def global_masking_threshold(self, mdct_amplitudes, tonality_per_block, drown = 0):
        ...
        Args:
            mdct_amplitudes (tensor): mdct amplitudes (spectrum). Shape : [batches_n, blocks_n, filter_bands_n, channels_n]
            tonality_per_block (tensor): tonality vector associated with mdct_amplitudes. Shape : [batches_n, blocks_n, 1, channels_n]
            drown (float, optional): factor 0..1 to drown out audible sounds (0: no drowning, 1: fully drowned). defaults to 0

        Returns:
            tensor: masking threshold in amplitude. Never negative. Shape : [batches_n, blocks_n, filter_bands_n, channels_n]
        """
        ...
        global_masking_intensity_in_bark = torch.maximum(masking_intensity, self.quiet_threshold_intensity)
    ...
    def add_noise(self, mdct_amplitudes, masking_threshold):
        ...
        Args:
            mdct_amplitudes (tensor): mdct amplitudes for each filter. Shape : [batches_n, blocks_n, filter_bands_n, channels_n]
            masking_threshold (tensor)): masking threshold in amplitude. Always positive. Shape : Shape : [batches_n, blocks_n, filter_bands_n, channels_n]

        Returns:
            tensor : mdct amplitudes with inaudible noise added
        """
        noise = masking_threshold * torch.normal(torch.zeros_like(masking_threshold),torch.ones_like(masking_threshold)/6)      
    ...
    def _masking_intensity_in_bark(self, mdct_amplitudes,tonality_per_block, drown = 0.0):
        ...
        Args:
            ...
            tonality_per_block (tensor): tonality vector associated with the mdct_amplitudes. Shape : [batches_n, blocks_n, 1, channels_n]
                                        Should be of compute_dtype
            drown (float, optional): factor 0..1 to drown out audible sounds (0: no drowning, 1: fully drowned). defaults to 0.

        Returns:
            tensor: vector of intensities for softest audible sounds given a certain sound
        ...
        masking_matrix = torch.einsum('ij,nbjc->nbijc',
        ...
        masking_intensity_in_bark = torch.einsum('nbic,nbijc->nbjc',
                                                torch.pow(torch.maximum(self._INTENSITY_EPS, intensities_in_bark), self.alpha),
                                                masking_matrix)
        masking_intensity_in_bark = torch.pow( torch.maximum(self._INTENSITY_EPS, masking_intensity_in_bark),1. / self.alpha)
    ...
    def _spreading_matrix_in_bark(self):
        ...
        z = torch.linspace(-self.max_bark, self.max_bark, 2 * self.bark_bands_n)
        f_spreading = 15.81 + 7.5 * (z + 0.474) - 17.5 * torch.sqrt(1 + torch.pow(z + 0.474,2))

        # Convert from dB to intensity and include alpha exponent
        f_spreading_intensity = torch.pow(10.0, self.alpha * f_spreading / 10.0)

        # Turns the spreading prototype function into a (bark_bands_n x bark_bands_n) matrix of shifted versions.
        # Transposed version of (9.17) in Digital Audio Signal Processing by Udo Zolzer
        spreading_matrix = torch.stack([f_spreading_intensity[(self.bark_bands_n - row):(2 * self.bark_bands_n - row)]
    ...
    def _quiet_threshold_intensity_in_bark(self, precompute_dtype):
        ...
        Returns:
            tensor : intensity vector for softest audible sounds [1, 1, bark_bands_n, 1]
        ...
        quiet_threshold_dB = torch.clip(
          ...
          self._dB_MIN.type(precompute_dtype), self._dB_MAX.type(precompute_dtype))
        # convert to amplitude scale, where _dB_MAX corresponds with an amplitude of 1.0
        quiet_threshold_intensity = torch.pow(torch.tensor(10.0, dtype=precompute_dtype),
    ...
    def _bark_freq_mapping(self, precompute_dtype):
        ...
        def freq_interval_overlap(freq_index, bark_index):
          ...
          bark_low_in_Hz = torch.broadcast_to(self.bark2freq(bark_low), [self.filter_bands_n, self.bark_bands_n])
          bark_high_in_Hz = torch.broadcast_to(self.bark2freq(bark_low + self.bark_band_width), [self.filter_bands_n, self.bark_bands_n])

          freq_low = filter_band_width * freq_index
          bark_low_in_Hz_clipped = torch.clip(bark_low_in_Hz, freq_low, freq_low + filter_band_width)
          bark_high_in_Hz_clipped = torch.clip(bark_high_in_Hz, freq_low, freq_low + filter_band_width)
    ...
    def _to_bark_intensity(self, mdct_amplitudes):
        ...
        Args : 
            param mdct_amplitudes(tensor) :  vector of mdct amplitudes (spectrum) for each filter [batches_n, blocks_n, filter_bands_n, channels_n]
            param W(tensor) :                matrix to convert from filter bins to bark bins [filter_bands_n, bark_bands_n]
            
        Returns :
            tensor : vector of intensities per Bark band [batches_n, blocks_n, bark_bands_n, channels_n]
        """
        # tf.maximum() is necessary, to make sure rounding errors don't make the gradient nan!
        mdct_intensity = torch.pow(mdct_amplitudes, 2)
        mdct_intensity_in_bark = torch.einsum('nbic,ij->nbjc', mdct_intensity, self.W)
    ...
    def _bark_intensity_to_freq_ampl(self, bark_intensity):
        ...
        Args : 
            param bark_intensity(tensor) :   vector of signal intensities in the Bark bands [batches_n, blocks_n, bark_bands_n, channels_n]
            param W_inv(tensor) :            matrix to convert from filter bins to bark bins [bark_bands_n, filter_bands_n]
            
        Returns:                 
            (tensor) : vector of mdct amplitudes (spectrum) for each filter [batches_n, blocks_n, filter_bands_n, channels_n]
        """
        mdct_intensity = torch.einsum('nbic,ij->nbjc', bark_intensity, self.W_inv)
        return torch.pow(torch.maximum(self._INTENSITY_EPS, mdct_intensity), 0.5)
